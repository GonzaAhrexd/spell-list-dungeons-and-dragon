...
function RunesManage() {
  const { nivelActual, setArmonicRunes, selectedCharacter } =
  ...
  useEffect(() => {
    if (backendRunes && Array.isArray(backendRunes)) {
      ...
      const backendArmónicas = backendRunes.filter(
        (r: any) => r.tipoRuna === "Armónica",
      );
  ...
  if (isLoading)
    return <div className="p-4 text-black">Invocando runas...</div>;
            ...
            {runes.map((rune) => (
              <label
                ...
                className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors ${
                  selectedRunes[level] === rune.nombre
