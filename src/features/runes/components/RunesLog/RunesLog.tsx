...
import RunesManage from "../RunesManage/RunesManage";
...
function RunesLog() {
  ...
  const runasTotales =
    selectedCharacter?.runasPorNivel?.[nivelActual]?.runasTotales || 0;
  const runasMaximasPorCombate =
    selectedCharacter?.runasPorNivel?.[nivelActual]?.runasActivas || 0;
  const porcentajeRunas =
    runasTotales > 0
      ? Math.round((runasActivas / runasMaximasPorCombate) * 100)
      : 0;
