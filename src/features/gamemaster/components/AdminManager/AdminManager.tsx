...
import ChangeCharacter from "@/features/characters/components/ChangeCharacter/ChangeCharacter";
import AddSpellScreen from "@/features/spells/components/AddSpell/AddSpellScreen";
...
function AdminManager({ setSeeSelectCharacter }: AdminManagerProps) {
  ...
  if (isLoading) {
    return (
      <div className="mobile-shell w-full max-w-[420px] mx-auto p-4">
        <p>Cargando panel...</p>
      </div>
    );
  } else
      ...
      <main className="mobile-shell w-full max-w-[420px] mx-auto">
        <header className="p-4">
          ...
          <p className="text-sm text-white/70">
            Gestión de hechizos (agregar y revisar)
          </p>
        </header>
        ...
        {view === "add" && (
          <div>
            <div className="p-4">
              <button
                className="mb-3 text-sm cursor-pointer text-gray-200"
          ...
          <div>
            {!showingSpellList && (
              <div>
                <div className="p-4">
                  <button
                    className="mb-3 text-sm text-gray-200 cursor-pointer"
                    onClick={() => setView("home")}
                  >
                    ← Volver
                  </button>
