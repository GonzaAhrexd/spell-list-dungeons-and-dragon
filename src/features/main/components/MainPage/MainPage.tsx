...
// Componentes
import SpellList from "../../../spells/components/SpellsList/SpellList";
import Consumptions from "../../../spells/components/Consumptions/Consumptions";
import HeaderApp from "../Header/HeaderApp";
import SpellsMenu from "@/features/spells/components/SpellsMenu/SpellsMenu";
import ChangeCharacter from "../../../characters/components/ChangeCharacter/ChangeCharacter";
import SelectCharacter from "../../../characters/components/SelectCharacterPage/SelectCharacter";
import RunesLog from "../../../runes/components/RunesLog/RunesLog";
import RunesList from "@/features/runes/components/RunesList/runesList";
import AdminManager from "@/features/gamemaster/components/AdminManager/AdminManager";
...
function MainPage() {
  ...
  const [seeSelectCharacter, setSeeSelectCharacter] = useState(
    selectedCharacter.personaje === "none",
  );
  ...
  const handleServer = () => {
    Swal.fire({
      ...
      text: isOnline
        ...
        : "El servidor está inactivo, pueden haber retrasos en cargar los hechizos locales, y los hechizos de la base de datos NO están disponibles. 🔴",
    ...
    <div className="app-viewport min-h-screen flex items-center justify-center p-4">
      ...
      {!seeSelectCharacter && selectedCharacter.personaje !== "Game Master" && (
        <>
          ...
          {mapMode && <MapMode onClose={() => setMapMode(false)} />}
          <main className="mobile-shell w-full max-w-[420px] mx-auto">
            <HeaderApp
              setIsTiradasModal={setIsTiradasModal}
              setMapMode={setMapMode}
            />

            <div className="flex justify-between">
              <button
                className="antiqua-font w-full cursor-pointer mr-1 mb-2 level-card relative flex items-center justify-center py-4 px-3 text-center text-sm font-bold shadow-inner"
                onClick={handleSpellsMenu}
              >
                {canUseSpells
                  ? "Hechizos"
                  : isRunicMode
              ...
              {canUseSpells && (
                <button
                  className="antiqua-font w-full cursor-pointer mb-2 level-card relative flex items-center justify-center py-4 px-3 text-center text-sm font-bold shadow-inner"
                  onClick={handleConsumptionMenu}
                >
                  Consumo
                </button>
              )}
            ...
            {!showingSpellList && showConsumptionMenu && <Consumptions />}
            ...
            {showRunesSettings && <RunesLog />}
            {showRunesList && !showRunesSettings && (
              <RunesList
                level={selectedLevel}
                onBack={() => {
                  setShowRunesList(false);
            ...
            >
              Estado del servidor: {isOnline ? "Activo 🟢" : "Inactivo 🔴"}
            </div>
          </main>
        </>
      )}
