      ...
      >
        ...
        <section className="mt-3 text-sm leading-relaxed space-y-3">
          <p className="regular-font text-base text-gray-800">
            Esta es una guía rápida sobre qué tiradas corresponden con cada
            atributo en el juego
          </p>
        </section>
        ...
        <section className="mt-4 max-h-80 overflow-y-auto">
          <ul className="space-y-3">
            {TiradasList.map((tirada, index) => (
              <li
                key={index}
                className="p-3 border border-gray-300 rounded-lg cursor-pointer level-card relative flex flex-col items-center justify-center py-4 px-3 text-center text-sm font-medium shadow-inner"
              >
