...
import SpellShow from "../SpellShow/SpellShow";
...
function SpellList({
  ...
  const characterSpells = data.personajes.find(
    (p) => p.personaje === selectedCharacter.personaje,
  );
  ...
  useEffect(() => {
    ...
    setSpellsState((prev) => {
      const backendNames = new Set(
        backendSpells.map((b: JSONSpell) => b.nombre),
      );
      // Filtra el JSON local eliminando duplicados
      const localSinDuplicados = prev.filter(
        (p) => !backendNames.has(p.nombre),
      );
      // Une todo
    ...
    return (
      <section className="parchment p-4 mt-4">
        <div className="text-black">
          Error al cargar hechizos: {(error as Error).message}
        </div>
      </section>
    );
  }
      ...
      <div className="spell-list text-black">
        <div className="back-wrapper mb-3">
          ...
          <div className="flex flex-row justify-center items-center gap-2">
            <button
              className="spell-button cursor-pointer w-full text-xs text-left px-3 py-2 rounded-lg bg-white/90 dark:bg-black/10 shadow-sm flex flex-col justify-start items-start"
              onClick={() => onPreviousLevel()}
            >
              <ArrowLongLeftIcon className="h-6 w-6 text-black items-center justify-center" />{" "}
              Nivel Anterior
            </button>
            <button
              className="spell-button cursor-pointer w-full text-xs text-left px-3 py-2 rounded-lg bg-white/90 dark:bg-black/10 shadow-sm flex flex-col justify-end items-end"
              onClick={() => onNextLevel()}
            >
              <ArrowLongRightIcon className="h-6 w-6 text-black items-end justify-end" />{" "}
              Nivel Siguiente
            </button>
          </div>
        </div>
              ...
              <div className="potencia-groups mt-4 space-y-4">
                {selectivos.length > 0 && (
                  <section className="potencia-group">
                    <h4 className="potencia-title text-sm font-semibold mb-2">
                      Selectivo
                    </h4>
