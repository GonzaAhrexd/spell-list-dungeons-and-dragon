      ...
      <div className="bg-parchment/95 backdrop-blur-sm rounded-xl shadow-lg p-6 max-w-2xl mx-auto">
        <header className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-black/70">
              Completa los datos para registrar un nuevo hechizo.
            </p>
          </div>
        </header>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
          ...
          <div>
            ...
            <select
              className="mt-1 w-full rounded-lg px-3 py-2 bg-white/80 border border-black/10 text-black focus:ring-amber-500"
          ...
          <div
            className={
              "grid gap-4" + (!isRuneMode ? " grid-cols-2" : "grid-cols-1")
            }
          >
            {!isRuneMode && (
              <div>
                ...
                <select
                  className="mt-1 w-full rounded-lg px-3 py-2 bg-white/80 border border-black/10 focus:ring-amber-500 placeholder-gray-500 text-black"
                  {...register("potencia", {
                    valueAsNumber: true,
                    required: false,
                  })}
            ...
            {isRuneMode && (
              <div>
                ...
                <select
                  className="mt-1 w-full rounded-lg px-3 py-2 bg-white/80 border border-black/10 focus:ring-amber-500 placeholder-gray-500 text-black"
                  {...register("tipoRuna", { required: false })}
                >
          ...
          <div>
            <label className="block text-sm font-semibold text-amber-950 mb-2">
              Usuarios (seleccionar uno o varios)
            </label>
                ...
                <>
                  {!isRuneMode
                    ? p.subclase != "Rúnico" && (
                        <label
                          key={p.personaje}
                          className="flex items-center gap-3 text-black bg-white/10 px-3 py-2 rounded-md"
                        >
                          <input
                            className="w-4 h-4"
                            type="checkbox"
                            value={p.personaje}
                            {...register("usuariosConHechizo")}
                          />
          ...
          <div className="flex gap-3 justify-end">
            <button
              ...
              onClick={() => setAddSpell(false)}
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="px-4 py-2 rounded-lg bg-amber-600 text-white font-semibold hover:bg-amber-500"
            >
              Agregar Hechizo
            </button>
          </div>
