...
function Consumptions() {
  ...
  const displayCount = (c: number, max: number) =>
    max == Infinity ? "∞" : `${c}/${max}`;
  ...
  if (!seeHistorial) {
    return (
      <section className="parchment p-4">
        <div className="bg-parchment/95 backdrop-blur-sm rounded-xl shadow-lg p-3">
                ...
                <div key={item.p} className="flex items-center gap-3">
                  <div className="w-28">
                    ...
                    <div className="text-[13px] text-gray-700">
                      {displayCount(
                        item.count,
                        selectedCharacter.limitePotencias[item.p],
                      )}
                    </div>
                  </div>
                        ...
                        return (
                          <button
                            ...
                            onClick={() =>
                              !disabled &&
                              spendSpell &&
                              spendSpell(item.p, `Manual ${item.p}`, true)
                            }
                        ...
                        return (
                          <button
                            ...
                            onClick={() =>
                              !disabled &&
                              spendSpell &&
                              spendSpell(item.p, `Manual ${item.p}`, false)
                            }
                            aria-label={`Usar hechizo potencia ${item.p}`}
                            disabled={disabled}
                          >
          ...
          <button
            className="bg-amber-950 w-full rounded-lg mt-2 cursor-pointer"
            onClick={() => setSeeHistorial(true)}
          >
    ...
    return (
      <SpellLog
        setSeeHistorial={setSeeHistorial}
        historialHechizos={historialHechizos}
