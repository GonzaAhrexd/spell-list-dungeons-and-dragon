...
function SpellShow({ spell, onClose }: SpellShowProps) {
  ...
  const [selectedPotencia, setSelectedPotencia] = useState<number>(spell?.potencia ?? 1)
      ...
      <article className="modal-sheet relative w-full  max-w-[420px] bg-white rounded-2xl shadow-2xl p-4 pb-6 text-black backdrop-blur-sm">
        <header className="flex items-start justify-between">
          <div>
            ...
            <div className="text-sm opacity-80">{capitalizeFirstLetter(spell.tipo)}</div>
            <div className="text-sm opacity-80">Nivel {spell.level} {isSelectivo ? '· Selectivo' : `· Potencia ${spell.potencia}`}</div>
          </div>
        ...
        <footer className="mt-4 flex gap-3">
          ...
          <div className='flex flex-col w-full'>
            <div className='flex flex-row items-center gap-3 p-2'>
              <label className='text-sm font-medium text-black'>Potencia</label>
              <select
            ...
            <div className='flex gap-3'>
                ...
                {(() => {
                  ...
                  const disponible = p === 1 || (p === 2 ? potencia2 > 0 : p === 3 ? potencia3 > 0 : p === 4 ? potencia4 > 0 : false)
                  return (
